  ...
  try {
    ...
    try {
      ...
      results.tests.push({
        ...
        success: !usersError,
        data: usersData?.map(u => ({ id: u.id, email: u.email?.substring(0, 3) + '***', role: u.role })) || [],
    ...
    console.log('🔍 Testing raw HTTP API access...');
    try {
      const { env } = await import('@/lib/config/env');
      
      const response = await fetch(`${env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/users?select=id,role&limit=2`, {
        headers: {
