...
export async function POST() {
  try {
    ...
    const edgeFunctionResponse = await fetch(`${process.env.SUPABASE_URL}/functions/v1/portfolio-image-sync`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json({
      ...
      portfolioTested: {
        ...
        beforeImagesCount: Array.isArray(portfolio.before_images) ? portfolio.before_images.length : 0,
