...
export async function GET() {
  try {
    console.warn("Debug: Fetching featured portfolio projects...");
    const projects = await getPortfolioProjects({ featuredOnly: true });

    console.warn("Debug: Raw projects from database:", JSON.stringify(projects, null, 2));
    // Transform the data like the homepage does
    const transformedData = projects.map((project) => ({
      id: project.id,
      title: project.title,
      description:
        project.brief_description || project.description || "Beautiful furniture transformation",
      beforeImage:
