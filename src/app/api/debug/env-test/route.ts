...
export async function GET() {
  try {
    ...
    const envInfo = {
      ...
      NEXT_PUBLIC_SUPABASE_ANON_KEY_FIRST_10:
        env.NEXT_PUBLIC_SUPABASE_ANON_KEY?.substring(0, 10) || "missing",
      SUPABASE_SERVICE_ROLE_KEY_LENGTH: env.SUPABASE_SERVICE_ROLE_KEY?.length || 0,
      SUPABASE_SERVICE_ROLE_KEY_FIRST_10:
        env.SUPABASE_SERVICE_ROLE_KEY?.substring(0, 10) || "missing",
      NODE_ENV: process.env.NODE_ENV,
      // Raw environment variables for comparison
      RAW_ANON_KEY_LENGTH: process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY?.length || 0,
      RAW_ANON_KEY_FIRST_10:
