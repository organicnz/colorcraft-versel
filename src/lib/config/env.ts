  ...
  try {
    ...
    if (process.env.NODE_ENV === "production") {
      ...
      if (usingDefaults.length > 0) {
        console.warn(
          `⚠️ Warning: Using default values for the following environment variables in production: ${usingDefaults.map(([key]) => key).join(", ")}`
        );
      }
    }
    ...
    if (error instanceof z.ZodError) {
      const missingEnvVars = error.errors.map((err) => err.path.join(".")).join(", ");
      ...
      if (process.env.NODE_ENV === "production") {
        console.error(
          "Environment error in production. Using fallback values to prevent deployment failure."
        );
      }
    } else {
    ...
    return {
      ...
      NEXT_PUBLIC_EMAIL_FROM:
        process.env.NEXT_PUBLIC_EMAIL_FROM || "Color & Craft <contact@colorcraft.live>",
