...
export function useChat() {
        ...
        setState((prev) => ({
          ...
          unreadCount:
            data.conversations?.reduce(
              (acc: number, conv: ChatConversationWithDetails) => acc + (conv.unread_count || 0),
              0
            ) || 0,
        }));
      } else {
  ...
  const createConversation = useCallback(
    async (conversation: NewChatConversation) => {
      try {
        const response = await fetch("/api/chat/conversations", {
          method: "POST",
          headers: {
  ...
  const setCurrentConversation = useCallback(
    async (conversation: ChatConversationWithDetails) => {
