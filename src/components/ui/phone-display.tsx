    ...
    <div className={cn(baseClasses, containerClasses[variant], className)}>
      ...
      <div className="flex items-center gap-2 sm:gap-3 w-full justify-center min-w-0 max-w-full">
        ...
        <a
          ...
          className={cn(variantClasses[variant], "hover:scale-105 transition-transform min-w-0 max-w-full overflow-hidden text-ellipsis whitespace-nowrap sm:whitespace-normal")}
        >
          {phoneNumber}
        </a>
      </div>
      ...
      {variant === 'hero' && (
        <>
          ...
          <div className="absolute -bottom-1 -left-1 w-2 h-2 bg-[#38BC81] rounded-full opacity-40 animate-pulse" style={{ animationDelay: '0.5s' }}></div>
        </>
      )}
    </div>
  );
